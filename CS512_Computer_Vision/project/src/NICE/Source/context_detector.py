...
class ContextDetector():
    ...
    def train_quantizers(self, network: Any, train_episode: Any, episode_index: int):
        network.eval()
        subsets = get_n_samples_per_class(train_episode, self.args.memo_per_class_context, self.task2classes, episode_index)
        with torch.no_grad():
    ...
    def push_activations(self, network: Any, train_episode: Any, episode_index: int):
        if episode_index == 1:
            self.train_quantizers(network, train_episode, episode_index)
        # Binary context Part
        network.eval()
        ...
        with torch.no_grad():
            for samples, class_ in subsets:
                ...
                if len(is_conv) != len(context_layer_activations):
                    raise Exception("We did not test using layers partially for context")
...
def get_n_samples_per_class(dataset, n: int, task2classes: dict, episode_index: int) -> List:
    classes_in_this_experience = task2classes[episode_index]
    indices = {i: [] for i in classes_in_this_experience}
    for i, (_, y) in enumerate(dataset.dataset):
        if y in indices:
            indices[y].append(i)

    subsets = []
    for i in classes_in_this_experience:
