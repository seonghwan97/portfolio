...
class CNN_Simple(nn.Module):
    def __init__(self, args: Namespace, input_size: int, output_size: int) -> None:
        ...
        self.conv_layers_early.extend([
            SparseConv2d(self.input_size, 64, 3, stride=1,layer_name="conv_early"), nn.ReLU(), nn.MaxPool2d(2),
            SparseConv2d(64, 64, 3, stride=1, layer_name="conv_early"), nn.ReLU(), nn.MaxPool2d(2),
            SparseConv2d(64, 64, 3, stride=1, layer_name="conv_early"), nn.ReLU(), nn.MaxPool2d(2)
            ])
        self.conv_layers_late.extend([
            SparseConv2d(64, 128, 3, stride=1, layer_name="conv_late"), nn.ReLU(), nn.MaxPool2d(2),
        ...
        self.penultimate_layer = nn.ModuleList([
            SparseLinear(1024, 1024, layer_name="linear"), nn.ReLU()
            ])
        ...
        for m in self.modules():
            if isinstance(m, (SparseLinear, SparseConv2d, SparseOutput)):
                unit_ranks_list.append(
                    ([0]*m.weight.data.shape[0], m.layer_name))
    ...
    def reinit_young(self):
        ...
        for m in self.modules():
            if isinstance(m, SparseLinear) or isinstance(m, SparseConv2d) or isinstance(m, SparseOutput):
                m.weight.data[self.current_young_neurons[i+1], :] = nn.init.kaiming_normal_(
                    m.weight.data[self.current_young_neurons[i+1], :], 
                    mode='fan_out', 
                    nonlinearity='relu'
                    )
                
                m.bias.data[self.current_young_neurons[i+1]] = nn.init.constant_(
                    m.bias.data[self.current_young_neurons[i+1]], 
                    0.0
                    )
                i += 1
        ...
        for module in self.modules():
            if isinstance(module, SparseLinear) or isinstance(module, SparseConv2d) or isinstance(module, SparseOutput):
                ...
                weights.append((copy.deepcopy(weight_mask).cpu().numpy(),
                                copy.deepcopy(bias_mask).cpu().numpy()))
        return weights
        ...
        for module in self.modules():
            if isinstance(module, SparseLinear) or isinstance(module, SparseConv2d) or isinstance(module, SparseOutput):
                if module.weight.requires_grad:
                    ...
                    if module.bias_flag:
                        module.bias.grad[self.freeze_masks[mask_index][1]] = 0
                mask_index = mask_index + 1

            if isinstance(module, nn.BatchNorm1d) or isinstance(module, nn.BatchNorm2d):
                if module.frozen_units is not None and module.affine:
                    module.weight.grad = torch.where(module.frozen_units, torch.zeros_like(module.weight.grad), module.weight.grad)
                    module.bias.grad = torch.where(module.frozen_units, torch.zeros_like(module.bias.grad), module.bias.grad)
...
class VGG11_SLIM(CNN_Simple):
    ...
    def __init__(self, args: Namespace, input_size: int, output_size: int) -> None:
        ...
        self.freeze_masks = []
        unit_ranks_list = [([999] * 3, "input")]
        for m in self.modules():
            if isinstance(m, (SparseLinear, SparseConv2d, SparseOutput)):
                unit_ranks_list.append(([0]*m.weight.data.shape[0], m.layer_name))
