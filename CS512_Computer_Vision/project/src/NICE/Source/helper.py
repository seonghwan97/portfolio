...
def random_prune(network: Any, pruning_perc: float, skip_first_conv=True) -> nn.Module:
    ...
    for module in network.modules():
        if isinstance(module, (SparseLinear, SparseOutput)):
            weight_masks.append(torch.from_numpy(np.random.choice([0, 1], module.weight.shape, p=[pruning_perc, 1 - pruning_perc])))
            bias_masks.append(torch.ones(module.bias.shape))
        if isinstance(module, SparseConv2d):
            connectivity_mask = torch.from_numpy(np.random.choice([0, 1], (module.weight.shape[0], module.weight.shape[1]), p=[0, 1] if first_conv_flag else [pruning_perc, 1 - pruning_perc]))
            ...
            filter_masks = [[np.ones(kernel_shape) if connectivity_mask[out_index, in_index] else np.zeros(kernel_shape) for in_index in in_range] for out_index in out_range]
